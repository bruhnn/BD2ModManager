...
class ThemeManager:
    ...
    @classmethod
    def icon(cls, icon_name: str, size=QSize(24,24)):
        icon_color = cls.THEMES[cls.current_theme].get("icon_color")
        if icon_color:
            return cls.recolored_icon(icon_name, icon_color, size)
        path = f":/material/{cls.current_theme}/{icon_name}.svg"
        return QIcon(path)

    @icon.register
    @classmethod
    def _(cls, widget: QWidget):
        return cls.icon(widget.property("iconName"))
