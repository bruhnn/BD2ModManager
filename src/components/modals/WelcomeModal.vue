...
async function handlePathSelected(path: string) {
    try {
        ...
        if (!isValid) {
            setError(t('app.modals.welcome.selectGameDirectory.errors.invalidDirectory', { path }))
            return
        }
        ...
        setError(t('app.modals.welcome.selectGameDirectory.errors.unknownError', { error: err }))
    }
}
...
async function handleDirectoryBrowse() {
    ...
    if (typeof file === 'string') {
        if (!file.toLowerCase().endsWith('browndust ii.exe')) {
            setError(t('app.modals.welcome.selectGameDirectory.errors.invalidGameExecutable'))
            return
        }
...
<template>
    <Modal :show="isOpen" size="lg" @close="closeModal" :close-on-escape="false"
        :title="$t('app.modals.welcome.title')" :subtitle="$t('app.modals.welcome.subtitle')">
        <div class="w-full h-full flex flex-col gap-4 p-4">
            <div v-if="!isChineseLanguage"
                ...
                <div class="flex flex-col gap-0.5">
                    <p class="text-xs font-semibold text-orange-300">{{ $t('app.modals.welcome.supportBanner.title') }}</p>
                    <p class="text-xs text-orange-200/80">{{ $t('app.modals.welcome.supportBanner.message', {platform: 'Ko-Fi'}) }}</p>
                </div>
            </div>
            ...
            <div v-else
                ...
                <div class="flex flex-col gap-0.5">
                    <p class="text-xs font-semibold text-purple-300">{{ $t('app.modals.welcome.supportBanner.title') }}</p>
                    <p class="text-xs text-purple-200/80">{{ $t('app.modals.welcome.supportBanner.message', { platform: 'AfDian' }) }}</p>
                </div>
            </div>

            <div class="flex items-center justify-ceter gap-2">
                <span @click="openUrl(GITHUB_URL)"
                    class="text-sm flex items-center gap-1.5 text-text-primary bg-surface-card border border-border-default rounded-full px-3 py-1 hover:text-text-primary! hover:bg-state-hover! cursor-pointer transition-colors">
                    <RefinedGithub class="w-4 h-4 fill-text-primary" />
                    {{ $t('app.modals.welcome.chips.github') }}
                </span>
                <span @click="openUrl(DISCORD_URL)"
                    class="text-sm flex items-center gap-1.5 text-text-primary bg-surface-card border border-border-default rounded-full px-3 py-1 hover:text-text-primary! hover:bg-state-hover! cursor-pointer transition-colors">
                    <DiscordIcon class="w-4 h-4 fill-text-primary" />
                    {{ $t('app.modals.welcome.chips.discord') }}
                </span>
            </div>

            <div class="flex flex-col gap-3">
                <div class="flex flex-col gap-1">
                    <p class="text-sm font-medium text-text-primary">{{ $t('app.modals.welcome.selectGameDirectory.title') }}</p>
                    <p class="text-xs text-text-secondary">
                        {{ $t('app.modals.welcome.selectGameDirectory.description') }}
                    </p>
                </div>

                <div class="flex gap-2">
                    <Input readonly :model-value="settings.gameDirectory ?? ''"
                        :placeholder="$t('app.modals.welcome.selectGameDirectory.placeholders.gamePath')" @click="handleDirectoryBrowse" />
                ...
                <div v-if="gamePathsFound.length > 0" class="flex flex-col gap-2">
                    <div class="flex justify-between items-center">
                        <p class="text-sm font-medium text-text-primary">
                            {{ $t('app.modals.welcome.selectGameDirectory.labels.foundPaths') }}
                        </p>
            ...
            <div class="flex justify-between shrink-0 p-2 px-4">
                <div v-if="settings.gameDirectory && !errorMessage" class="flex items-center gap-2">
                    ...
                    <span class="text-success text-sm">
                        {{ $t('app.modals.welcome.selectGameDirectory.validPath') }}
                    </span>
                </div>
                <div v-else></div>
                ...
                <Button @click="closeModal" variant="primary" :label="$t('common.actions.continue')" />
            </div>
        </template>
    </Modal>
</template>
